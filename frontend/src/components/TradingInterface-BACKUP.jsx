...
export default function TradingInterface({ partyId }) {
  ...
  React.useEffect(() => {
    const handleError = (event) => {
      ...
      console.error('[TradingInterface] Error stack:', event.error?.stack);
      console.error('[TradingInterface] Error filename:', event.filename);
      console.error('[TradingInterface] Error lineno:', event.lineno);
      console.error('[TradingInterface] Error colno:', event.colno);
    ...
    const handleUnhandledRejection = (event) => {
      ...
      console.error('[TradingInterface] Promise rejection stack:', event.reason?.stack);
      setErrorMessage(`Promise rejection: ${event.reason?.message || event.reason} (${event.reason?.stack?.split('\n')[0]})`);
      setHasError(true);
    };
  ...
  // Heartbeat checker to detect UI unresponsiveness
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const loadAvailablePairs = async () => {
      try {
        ...
        console.error('[TradingInterface] Error details:', err.message, err.stack);
        // Set default pairs to prevent UI from breaking
    ...
    const initializeData = async () => {
      try {
        ...
        await Promise.allSettled([
          loadBalance(true).catch(err => console.error('[TradingInterface] Balance load error:', err)),
          loadOrders().catch(err => console.error('[TradingInterface] Orders load error:', err)),
          loadOrderBook(true).catch(err => console.error('[TradingInterface] OrderBook load error:', err)),
      ...
      try {
        if (data?.tradingPair === tradingPair) {
          setTrades(prev => {
            ...
            return [data, ...currentTrades].slice(0, 100); // Keep last 100 trades
          });
          loadBalance(false).catch(err => console.error('[TradingInterface] Balance refresh error:', err)); // Refresh balance after trade
        }
      } catch (err) {
    ...
    try {
      ...
      console.error('[TradingInterface] WebSocket subscription error:', err);
    }
    ...
    const interval = setInterval(() => {
      try {
        ...
        loadOrderBook(false).catch(err => console.error('[TradingInterface] Polling OrderBook error:', err));
        loadOrders().catch(err => console.error('[TradingInterface] Polling orders error:', err));
        loadBalance(false).catch(err => console.error('[TradingInterface] Polling balance error:', err));
        loadTrades().catch(err => console.error('[TradingInterface] Polling trades error:', err));
      } catch (err) {
        console.error('[TradingInterface] Polling interval error:', err);
      }
    }, 5000);
  ...
  const loadOrders = useCallback(async () => {
    try {
      ...
      if (!globalOrderBook) {
        console.log('[Active Orders] No OrderBook found for', tradingPair);
        setOrders([]);
        return;
      }

      console.log('[Active Orders] OrderBook loaded:', globalOrderBook);

      const rawOrders = [
        ...(Array.isArray(globalOrderBook.buyOrders) ? globalOrderBook.buyOrders : []),
        ...(Array.isArray(globalOrderBook.sellOrders) ? globalOrderBook.sellOrders : [])
      ];

      console.log('[Active Orders] Raw orders:', rawOrders);
      ...
      // Don't set error state here to avoid breaking the UI, just log and continue
      setOrders([]);
    }
  }, [partyId, tradingPair]);
  ...
  const loadOrderBook = async (showLoader = false) => {
    ...
    try {
      ...
      const buyOrders = (Array.isArray(globalOrderBook.buyOrders) ? globalOrderBook.buyOrders : []).map(order => ({
        price: order.price,
      ...
      const sellOrders = (Array.isArray(globalOrderBook.sellOrders) ? globalOrderBook.sellOrders : []).map(order => ({
        price: order.price,
        quantity: order.quantity,
        filled: order.filled,
        remaining: order.remaining,
  ...
  // Early returns BEFORE all hooks to maintain consistent hook order
  if (error && !loading) {
    return (
      <div className="flex items-center justify-center min-h-[400px]">
