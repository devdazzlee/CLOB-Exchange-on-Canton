...
export default function TradingInterface({ partyId }) {
  ...
  const [mobileTradeTab, setMobileTradeTab] = useState('chart');
      ...
      <div className="flex-1 min-h-0 overflow-hidden flex flex-col lg:block">
        ...
        <div className="lg:hidden flex-1 flex flex-col min-h-0 overflow-hidden">
          ...
          <div className="flex-1 min-h-0 overflow-hidden pb-16">
            ...
            <div className={cn("h-full flex flex-col overflow-hidden", (mobileTradeTab !== 'account' && mobileTradeTab !== 'portfolio' && mobileTradeTab !== 'mint' && mobileTradeTab !== 'transfers') && "hidden")}>
              <div className="flex-shrink-0 flex items-center gap-1 px-3 pt-2 pb-1">
                {[{ id: 'portfolio', label: 'Portfolio' }, { id: 'mint', label: 'Mint Assets' }, { id: 'transfers', label: 'Transfers' }].map(t => (
                  <button key={t.id} onClick={() => setMobileTradeTab(t.id)}
                    className={cn("px-3 py-1 text-[10px] font-black uppercase tracking-wider rounded-full border transition-all",
                      mobileTradeTab === t.id
                        ? "bg-[#2b3139] text-[#F7B500] border-[#F7B500]/40"
              ...
              <div className="flex-1 overflow-y-auto p-3">
                {mobileTradeTab === 'mint' ? (
                  <BalanceCard 
                    ...
                    onMintToken={(token, amount) => { if (!token) { handleMintTokens(); } else { handleMintTokens([{symbol: token, amount}]); } }} 
                    onRefresh={() => refreshAllData(tradingPair)} 
                  />
                ) : mobileTradeTab === 'transfers' ? (
              ...
              const isActive = tab.id === 'chart'
                ...
                : tab.id === 'book'
                  ...
                  : tab.id === 'orders'
                    ...
                    : tab.id === 'account'
                      ? ['account', 'portfolio', 'transfers', 'mint'].includes(mobileTradeTab)
