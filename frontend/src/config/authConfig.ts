 ...
 */
export const CANTON_CONFIG: CantonConfig = {
...
export interface JWTPayload {
  ...
  party?: string;     // Canton party ID (if included)
  act?: string[];     // Act-as parties
  read?: string[];    // Read-as parties
}
 ...
 * @param token - JWT token string
 * @returns Ledger ID or null
 */
export function getLedgerIdFromToken(token: string): string | null {
 ...
 * 
 * @param token - JWT token string
 * @returns Party ID prefix from token
 */
export function getPartyIdFromToken(token: string): string | null {
  ...
  // Full party ID format: "prefix::suffix" 
  // But we can't derive the suffix from the token alone
 ...
 * @param token - JWT token string
 * @returns true if token has daml_ledger_api scope
 */
export function hasCantonScope(token: string): boolean {
  const payload = decodeJWT(token);
 ...
 * @param endpoint - API endpoint (e.g., '/state/active-contracts')
 * @returns Full URL
 */
export function getCantonApiUrl(endpoint: string): string {
 ...
 */
export function getDefaultTradingPairs(): string[] {
