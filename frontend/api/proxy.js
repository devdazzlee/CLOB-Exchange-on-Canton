...
export default async function handler(req, res) {
  ...
  try {
    ...
    // SOLUTION: Use browser JWT first (auto-refreshes), fallback to static token
    let cantonToken = req.headers.authorization;
    
    // If no browser JWT, use static token as fallback
    if (!cantonToken) {
      ...
      console.log('[Proxy] Using browser JWT (auto-refreshes)');
    }
