...
class OnboardingService {
  ...
  async getCantonToken() {
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `Failed to get Canton token: ${response.status} - ${errorText}`,
      );
    }
  ...
  async discoverSynchronizerId() {
    ...
    if (config.canton.synchronizerId) {
      console.log(
        "[OnboardingService] Using synchronizerId from env:",
    ...
    if (
      this.cachedSynchronizerId &&
      this.synchronizerCacheExpiry &&
      Date.now() < this.synchronizerCacheExpiry
    ) {
    ...
    if (
      ...
      const globalSync = synchronizers.find(
        (s) => s.synchronizerAlias === "global" || s.alias === "global",
      );
      if (globalSync && globalSync.synchronizerId) {
        ...
        const globalDomainSync = synchronizers.find(
          (s) => s.synchronizerId && s.synchronizerId.includes("global-domain"),
        );
        if (globalDomainSync && globalDomainSync.synchronizerId) {
          ...
          synchronizerId =
            synchronizers[0].synchronizerId || synchronizers[0].id;
        }
      }
    }
    // Legacy support: check for synchronizers (plural, without "connected" prefix)
      ...
      synchronizerId =
        data.synchronizers[0].synchronizerId || data.synchronizers[0].id;
    }
    ...
    if (!synchronizerId) {
      throw new Error(
        "Could not extract synchronizerId from response: " +
          JSON.stringify(data),
      );
    }

    console.log(
      "[OnboardingService] Discovered synchronizerId:",
      synchronizerId,
    );
  ...
  async generateTopology(publicKeyBase64, partyHint) {
    ...
    const effectivePartyHint =
      partyHint && partyHint.trim() !== ""
        ? partyHint.trim()
        : this.generatePartyHint(publicKeyBase64);

    console.log(
      "[OnboardingService] Generating topology for partyHint:",
      effectivePartyHint,
    );
    ...
    console.log(
      "[OnboardingService] Generate-topology request:",
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `Generate topology failed (${response.status}): ${responseText}`,
      );
    }
    ...
    try {
      ...
      throw new Error(
        `Invalid JSON response from generate-topology: ${responseText}`,
      );
    }

    console.log(
      "[OnboardingService] Generate-topology response:",
      JSON.stringify(data, null, 2),
    );

    // FIX: Normalize response to handle both topologyTransactions and onboardingTransactions
    const topologyTransactions =
      data.topologyTransactions || data.onboardingTransactions || [];
    ...
    if (!topologyTransactions || topologyTransactions.length === 0) {
      throw new Error(
        "Generate-topology response missing topology transactions",
      );
    }
  ...
  async discoverIdentityProviderId() {
    ...
    if (!res.ok) {
      console.warn(
        "[Onboarding] Failed to list IDPs, using empty string:",
        res.status,
        await res.text(),
      );
    ...
    if (!idpId) {
      console.warn(
        "[Onboarding] Could not extract identityProviderId, using empty string",
      );
      return ""; // Fallback to empty string
    }
  ...
  async allocateParty(
    ...
    try {
      ...
      const onboardingTransactions = topologyTransactions.map((t) => ({
        transaction: t,
      }));
      ...
      console.log(
        "[OnboardingService] Allocate request (formatted):",
      ...
      console.log(
        "[OnboardingService] Allocate response:",
        JSON.stringify(data, null, 2),
      );
  ...
  async completeOnboarding(
    ...
    try {
      ...
      console.log(
        "[Onboarding] Party allocated successfully:",
        allocationResult.partyId,
      );

      // Step 2: Create UserAccount and mint tokens
      const tokenResult = await this.createUserAccountAndMintTokens(
        allocationResult.partyId,
      );
  ...
  async createUserAccountAndMintTokens(partyId) {
    try {
      ...
      const packageId = await cantonService.getPackageIdForTemplate(
        "UserAccount",
        adminToken,
      );
      ...
      console.log(
        "[Onboarding] Using templateId:",
        templateId.substring(0, 50) + "...",
      );
      ...
      console.error(
        "[Onboarding] Failed to create UserAccount or mint tokens:",
        error,
      );
  ...
  async mintTokens(partyId, tokenType, amount) {
    ...
    if (
      !faucetContracts.activeContracts ||
      faucetContracts.activeContracts.length === 0
    ) {
  ...
  async createPreapproval(partyId) {
    console.log(
      "[OnboardingService] Create-preapproval called for party:",
      partyId,
    );
