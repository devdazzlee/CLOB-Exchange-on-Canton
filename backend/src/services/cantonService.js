...
class CantonService {
  ...
  async getConnectedSynchronizers(token) {
    try {
      ...
      if (
        ...
        JSON.stringify(axiosErr.response?.data || {}).includes('security-sensitive')
      ) {
        console.warn(`[CantonService] Invalid token detected retrieving synchronizers. Invalidating cache.`);
        require('./tokenProvider').invalidateServiceToken();
      }
      throw classifyCantonError(axiosErr);
    }
  }
  ...
  async executeInteractiveSubmission({
    ...
    try {
      ...
      // Only refresh OAuth cache on real auth failures. Canton masks many denial reasons
      // (signatures, rights) behind "security-sensitive" — that must not clear the service token.
      if (axiosErr.response?.status === 401) {
        console.warn(`[CantonService] HTTP 401 during execute — invalidating cached service token.`);
        require('./tokenProvider').invalidateServiceToken();
      }
      const classified = classifyCantonError(axiosErr);
