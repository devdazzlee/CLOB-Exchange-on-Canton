...
class AutoAcceptService extends EventEmitter {
  ...
  async _autoAcceptTransfer(transfer) {
    ...
    try {
      ...
      // Idempotent completion: ledger/registry report no active contract — stop chasing this CID.
      if (err.code === 'CONTRACT_ARCHIVED' || msg.includes('archived') || msg.includes('expired')) {
        this._archived.add(contractId);
        return;
      }
      if (err.code === 'CONTRACT_NOT_FOUND' || msg.includes('CONTRACT_NOT_FOUND')) {
        this._archived.add(contractId);
        console.warn(
          `[AutoAccept] Idempotent: contract ${contractId.substring(0, 20)}... not active on ledger at prepare (already settled or race).`
        );
        return;
      }
  ...
  _extractTransferInfo(contract) {
    ...
    return {
      ...
      synchronizerId: contract.synchronizerId || contract.createdEvent?.synchronizerId || null,
    };
  }
  ...
  _extractTransferInfoFromCreatedEvent(createdEvent) {
    ...
    const synchronizerId =
      createdEvent.synchronizerId || createdEvent.synchronizer_id || null;
