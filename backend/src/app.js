...
async function startServer() {
  ...
  server.listen(PORT, '0.0.0.0', async () => {
    ...
    try {
      const { assertExecutorReadyForTransferInstructionAccept } = require('./services/executorLedgerRightsGuard');
      await assertExecutorReadyForTransferInstructionAccept();
      console.log('✅ Executor ledger rights OK for interactive TransferInstruction accept');
      ...
      if (error.name === 'ExecutorLedgerRightsError' || error.code === 'EXECUTOR_LEDGER_RIGHTS') {
        console.error('❌ Auto-Accept not started: executor ledger user lacks required rights.');
        console.error(`   ${error.message}`);
        console.error('   Fix: grant executor (Cardiv) CanReadAs for OPERATOR_PARTY_ID via gRPC/JSON API, then restart.');
      } else {
